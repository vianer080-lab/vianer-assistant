-- OAuth retrieval and four-channel publication queue support.
...
declare connection_metadata jsonb;
begin
  if p_service not in ('youtube','instagram','pinterest')
     or not private.master_hub_device_authorized(p_device_token) then
    return jsonb_build_object('authorized', false);
  end if;
  select (extensions.pgp_sym_decrypt(token_cipher, p_device_token)::text)::jsonb, metadata
    into result, connection_metadata
    from private.oauth_connections
   where service = p_service;
  if result is null then return jsonb_build_object('authorized', false); end if;
  return jsonb_build_object('authorized', true, 'token', result, 'metadata', coalesce(connection_metadata, '{}'::jsonb));
end;
$$;
...
begin
  if not private.master_hub_device_authorized(p_device_token) then return jsonb_build_object('ok',false,'error','unauthorized'); end if;
  if p_platform not in ('telegram','instagram','pinterest','youtube')
...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare platform text; declare result jsonb; declare jobs jsonb := '[]'::jsonb;
begin
  if coalesce(array_length(p_platforms,1),0) not between 1 and 4
  ...
  foreach platform in array p_platforms loop
    result := private.master_hub_enqueue_publication_v2_impl(p_device_token,platform,p_title,p_description,p_media_url,p_privacy_status,p_scheduled_at,p_options);
    if not coalesce((result->>'ok')::boolean,false) then raise exception using message=coalesce(result->>'error','queue_failed'); end if;
