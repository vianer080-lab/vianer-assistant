...
final class LiyaOfflineVoice implements RecognitionListener {
    ...
    private Model model; private SpeechService speech; private boolean active; private volatile boolean muted;
    ...
    // Do not pause/restart AudioRecord around every spoken answer: Samsung plays a
    // privacy sound whenever the microphone session is repeatedly reopened.
    void pause(boolean value){ muted=value; }
    ...
    private void accept(String json){ try { if(muted)return; String text=new JSONObject(json).optString("text","").toLowerCase(Locale.ROOT).trim(); if(text.isEmpty())return; if(text.contains("лия")&&text.contains("выключи голос")){active=false;state.accept("Голосовой режим выключен. Жду фразу включения.");return;} if(text.contains("лия")&&text.contains("включи голос")){active=true;state.accept("Голосовой режим включён. Слушаю вас.");return;} if(active)command.accept(text.replaceFirst("^лия[ ,]*","")); }catch(Exception ignored){} }
